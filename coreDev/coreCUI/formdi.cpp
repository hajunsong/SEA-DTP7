    ...
    ui(new Ui::FormDI)
{
    ...
    for(int i = 0 ; i < 32 ; i++)
    {
        connect(m_btnSet[i], SIGNAL(clicked()), this, SLOT(setDI()));
    }
...
void FormDI::showEvent(QShowEvent *)
{
    updateStatus();
    m_timer->start(200);
}

void FormDI::hideEvent(QHideEvent *)
{
    m_timer->stop();
}

void FormDI::updateStatus()
{
    ...
    for(int i = 0 ; i < 32 ; i++)
    {
        ...
        temp = QString("I%1").arg((i+1)+(m_idx*32));
...
void FormDI::on_btnPrev_clicked()
{
    if(m_idx == 0)
        m_idx = 3;
    else
...
void FormDI::on_btnNext_clicked()
{
    if(m_idx == 3)
        m_idx = 0;
    else
...
void FormDI::on_ckbOverRideDIn_clicked()
{
    CNRobo* probo = CNRobo::getInstance();

    bool boverride;
    probo->getOverrideDIN(boverride);

    if(ui->ckbOverRideDIn->isChecked())
    {
        if(!boverride)
           probo->setOverrideDIN(true);
    }
    else
    {
        if(boverride)
           probo->setOverrideDIN(false);
    }
}
